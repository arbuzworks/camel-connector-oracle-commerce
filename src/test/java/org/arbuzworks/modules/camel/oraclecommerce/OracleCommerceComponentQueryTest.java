...
public class OracleCommerceComponentQueryTest extends CamelTestSupport {
    ...
    public void testTimerInvokesBeanMethod() throws Exception {
		...
		for (Exchange e : exchanges) {
			Assert.assertTrue(e.getOut().getBody() instanceof String[]);
			String[] res = (String[])e.getOut().getBody();
			Assert.assertTrue(res != null && res.length == 1);
		}
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                from("oraclecommerce://73.47.102.249:8080/perform_r_q_l_query?" +
